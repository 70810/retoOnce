...
app.get('/polls', (req, res) =>
{
	muestraEncuesta(token, (err,data) =>
		{
         console.log(data);
         queryMysql(`select * from encuestas`, (error, opciones) => 
...
{	muestraEncuesta(token, (err,data) =>
		{
         console.log(data);
         queryMysql(`select * from encuestas where id = '${data.id}' and porcentaje<= 0`, (error, opciones) => 
	     res.json({{data}});
		});
});

app.put('/votePOll', (req, res) =>
// borrar una encuesta cambiando el estado ...
{   let token = req.param("id");  
	muestraEncuesta(token, (err,data) =>
		{
         console.log(data.id);
         queryMysql(`update encuestas set estado= '0' where id = '${data.id}'`, (error, opciones) => 
		      {	console.log(data);
...
// borrar una encuesta ...
{
	...
	muestraEncuesta(token, (err,data) =>
		{
		console.log(data.id);
		queryMysql(`delete  from encuestas where id = '${data.id}'`, (error, opciones) => 
			{
			queryMysql(`delete  from opciones where id = '${data.id}'`, (error, opciones ) => 
			{
				console.log(data);
				console.log(opciones);
				res.json({data});
...
{
	let encuesta = [];
	queryMysql(`select * from encuestas where id = '${token}'`, (err, pregunta) =>
	{
		if (err) throw err;
		if(pregunta.length !== 0)
		{
			...
			queryMysql(`select * from opciones where idEncuestas = ${pregunta[0].idEncuestas}`, (err, opciones) =>
			{
				if (err) throw err;
